...
```ad-done
By $\mathbb{E}|X|\ge a$, $\mathbb{P}[|X|\ge\lambda a]\ge\mathbb{P}[|X|\ge\lambda\mathbb{E}|X|]$. We have 
$$
\begin{align}
\mathbb{E}|X|&=\mathbb{E}|X|\mathbb{1}_{\{|X|\ge\lambda\mathbb{E}|X|\}}+\mathbb{E}|X|\mathbb{1}_{\{|X|<\lambda\mathbb{E}|X|\}}\\
&\le\lambda\mathbb{E}|X|+\big(\mathbb{P}[|X|\ge\lambda\mathbb{E}|X|]\big)^{\frac{1}{2}}\tag{Cauchy-Schwarz}\\
\end{align}
$$
Then 
$$
\mathbb{P}[|X|\ge\lambda\mathbb{E}|X|]\ge(1-\lambda)^2(\mathbb{E}|X|)^2\ge(1-\lambda)^2a^2
$$
```

```ad-question
title:21
Recall the notation $d(A, B) = P(A \Delta B)$ for events $A, B$. Prove $$ d(A_n, A) \to 0 \quad \text{iff} \quad 1_{A_n} \xrightarrow{L_2} 1_A. $$
```

```ad-done
Note that 
$$
|\mathbb{1}_{A_n}-\mathbb{1}_A|=\mathbb{1}_{A_n\bigtriangleup A}\Longrightarrow\mathbb{E}|\mathbb{1}_{A_n}-\mathbb{1}_A|^2=\mathbb{P}(A_n\bigtriangleup A)
$$
```

```ad-question
title:22
Suppose $\{X_n, n \geq 1\}$ are independent non-negative random variables satisfying $$ E(X_n) = \mu_n, \quad \operatorname{Var}(X_n) = \sigma_n^2. $$ Define for $n \geq 1$, $S_n = \sum_{i=1}^n X_i$ and suppose $\sum_{n=1}^\infty \mu_n = \infty$ and $\sigma_n^2 \leq c \mu_n$ for some $c > 0$ and all $n$. Show $S_n / E(S_n) \xrightarrow{P} 1$.
```
...
$$
\begin{align}
\mathbb{P}\left(\left|\frac{S_n}{\mathbb{E}(S_n)}-1\right|\ge\varepsilon\right)&\le\frac{1}{\varepsilon^2}\mathbb{E}\left|\frac{S_n}{\mathbb{E}(S_n)}-1\right|\\
&=\frac{Var(S_n)}{\varepsilon^2(\mathbb{E}(S_n))^2}\\
&=\frac{\sum_{k=1}^{n}\sigma_k^2}{\varepsilon^2(\sum_{k=1}^{n}\mu_k)^2}\le\frac{c}{\varepsilon^2\sum_{k=1}^{n}\mu_k}\to0
\end{align}
$$
```
...
```ad-question
title:23
A classical transform result says the following: Suppose $u_n \geq 0$ and $u_n \to u$ as $n \to \infty$. For $0 < s < 1$, define the generating function $$ U(s) = \sum_{n=0}^\infty u_n s^n. $$ Show that $$ \lim_{s \to 1} (1 - s) U(s) = u $$ by the following relatively painless method which uses convergence in probability: Let $T(s)$ be a geometric random variable satisfying $$ P[T(s) = n] = (1 - s)s^n. $$ Then $T(s) \xrightarrow{P} \infty$. What is $E(u_{T(s)})$?
```

```ad-done
Since $u_n\to u$, for $\varepsilon>0$, $\exists N\in\mathbb{N}$ s.t. $n\ge N$, $|u_n-u|<\varepsilon$. We take $\delta=\frac{\varepsilon}{2M}$ and $M=\max\{u_0,u_1,\cdots,u_N,u\}$. When $1-s<\delta$, 
$$
\begin{align}
\left|(1-s)U(s)-u\right|&=(1-s)\left|\sum_{n=0}^{\infty}u_ns^n-\sum_{n=0}^{\infty}us^n\right|\\
&\le(1-s)\sum_{n=0}^{\infty}|u_n-u|s^n\\
&\le(1-s)\sum_{n=0}^{N}|u_n-u|s^n+(1-s)\sum_{n=N+1}^{\infty}|u_n-u|s^n\\
&\le 2M\cdot\frac{\varepsilon}{2M}+\varepsilon\cdot s^{N+1}=\varepsilon(1+s^{N+1})
\end{align}
$$
We prove $\lim_{s\to1^+}(1-s)U(s)=u$. 
For any $M>0$, 
$$
\begin{align}
\mathbb{P}(T(s)>M)=\sum_{n=[M]+1}^{\infty}(1-s)s^n=s^{[M]+1}\to\mbox{ as }M\to\infty
\end{align}
$$
Hence, $T(s)\xrightarrow{\mathbb{P}}\infty$. 
$$
\mathbb{E}[u_{T(s)}]=\sum_{n=0}^{\infty}u_n(1-s)s^n=(1-s)U(s)
$$
```

```ad-question
title:24(a)
Recall a random vector $(X_n, Y_n)$ (that is, a random element of $\mathbb{R}^2$) converges in probability to a limit random vector $(X, Y)$ if $$ d((X_n, Y_n), (X, Y)) \xrightarrow{P} 0 $$ where $d$ is the Euclidean metric on $\mathbb{R}^2$. Prove $$ (X_n, Y_n) \xrightarrow{P} (X, Y) \tag{6.23} $$ iff $$ X_n \xrightarrow{P} X \quad \text{and} \quad Y_n \xrightarrow{P} Y. $$
```

```ad-done
For $\varepsilon>0$, note that 
$\Longleftarrow$: $$\{d((X_n,Y_n),(X,Y))\ge\varepsilon\}\subseteq\{|X_n-X|\ge\frac{\varepsilon}{2}\}\cup\{|Y_n-Y|\ge\frac{\varepsilon}{2}\}$$
$\Longrightarrow$: $$
\begin{align}
d&((X_n,Y_n),(X,Y))\ge|X_n-X|\ge\varepsilon\\
\Longrightarrow&\{|X_n-X|\ge\varepsilon\}\subseteq\{d((X_n,Y_n),(X,Y))\ge\varepsilon\}
\end{align}$$
```

```ad-question
title:24(b)
If $f : \mathbb{R}^2 \mapsto \mathbb{R}^d$ is continuous ($d \geq 1$), (6.23) implies $$ f(X_n, Y_n) \xrightarrow{P} f(X, Y). $$ 
```
...
```ad-question
title:24(c)
If (6.23) holds, then $$ (X_n + Y_n, X_n Y_n) \xrightarrow{P} (X + Y, XY). $$
```

```ad-done
Take continuous function $f(x,y)=(x+y,xy)$. 
```