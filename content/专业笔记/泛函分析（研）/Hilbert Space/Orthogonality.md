...
```ad-warning
- $\{tx + (1-t)y : 0\le t\le 1\}$ is s the straight-line segment joining $x$ and $y$
...
```ad-warning
Recall the [[Introduction#^45dc78|thm]], if $M$ is a closed linear subspace of $H$ and $h\in H$, then there is a unique element $f_0$ in $M$ such that $h-f_0\in M^{\bot}$. We can difine a map, in fact, we call it projection mapping
$$
