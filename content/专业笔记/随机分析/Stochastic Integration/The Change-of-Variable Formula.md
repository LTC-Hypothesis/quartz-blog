...
> Let $f:[0,\infty)\times \mathbb{R}^d\to\mathbb{R}$ and $f\in C^1[0,\infty)\cap C^2(\mathbb{R}^d)$. Suppose $\textbf{M}_t=(M_t^{(1)},\cdots,M_t^{(d)})$ and $\symbf{A}_t=(A_t^{(1)},\cdots,A_t^{(d)})$, $\textbf{X}_t$ is a continuous semimartingale vector and satisfies the decomposition $\symbf{X}_t=\symbf{X}_0+\symbf{M}_t+\symbf{A}_t$. Then it holds 
> $$
> \begin{align}
