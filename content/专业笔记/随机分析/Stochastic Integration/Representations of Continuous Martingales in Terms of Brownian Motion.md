...
```ad-def
title:Extension of a probability space 
...
```ad-thm
title:Time-change for martingales
...
**QED**
```ad-proposition
title:Ramification of time-change for martingale
...
1. $T$ is nondecreasing and right-continuous on $[0,S)$, with values in $[0, \infty)$. If $A(t) < S; \forall t \ge 0$, then $\lim_{s \uparrow S} T(s) = \infty$.
2. $A(T(s)) = s \wedge S; 0 \le s < \infty$. 
3. $T(A(t)) = \sup\{\tau \ge t: A(\tau) = A(t)\}; 0 \le t < \infty$. 
4. Suppose $\varphi: [0, \infty) \to \mathbb{R}$ is continuous and has the property $$ A(t_1) = A(t) \text{ for some } 0 \le t_1 < t \implies \varphi(t_1) = \varphi(t). $$ Then $\varphi(T(s))$ is continuous for $0 \le s < S$, and $$ \varphi(T(A(t))) = \varphi(t); \quad 0 \le t < \infty. $$
5. For $0 \le t, s < \infty$: $s < A(t) \Leftrightarrow T(s) < t$ and $T(s) \le t \Rightarrow s \le A(t)$.
6. If $G$ is a bounded, measurable, real-valued function or a nonnegative, measurable, extended real-valued function defined on $[a,b] \subset [0, \infty)$, then $$ \int_a^b G(t) dA(t) = \int_{A(a)}^{A(b)} G(T(s)) ds. $$
```
