...
```ad-def
title:Square-integrable martingale
...
1. $\langle \alpha X+\beta Y,Z\rangle_t=\alpha\langle X,Z\rangle_t+\beta\langle Y,Z\rangle_t,\forall\alpha,\beta\in\mathbb{R}$
2. $\langle X,Y\rangle_t=\langle Y,X\rangle_t$
3. $|\langle X,Y\rangle_t|^2\le \langle X\rangle_t\langle Y\rangle_t$
4. For $\mathbb{P}-a.s.\omega\in\Omega$, $0\le s<t<\infty$
   $$
...
**QED**
```ad-def
title:More conventional use of variation/p-variation
...
1. $M$ is a martingale, and $M$ and the submartingale $M^2$ are both uniformly integrable; in particular, $M_{\infty} = \lim_{t \to \infty} M_t$ exists a.s. $\mathbb{P}$, and $\mathbb{E} M_{\infty}^2 = \mathbb{E}\langle M \rangle_{\infty}$;
2. we may take a right-continuous modification of $Z_t = \mathbb{E}(M_{\infty}^2 | \mathcal{F}_t) - M_t^2$; $t \geq 0$, which is a potential.
```
