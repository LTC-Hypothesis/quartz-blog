...
> [!note] No.1 way to construct $d$-dimensional Brownian motion(Product Space)
...
> [!note] No.2 way to construct $d$-dimensional Brownian motion(Markov family)
...
> [!def] d-dimensional Brownian family
...
> [!thm] Markov property of Brownian motion
...
> [!def] Transfer semigroup operator
...
> [!def] d-dimensional Brownian motion with drift
...
> 4. For $E\in\mathcal{F}$, the mapping $x\mapsto \mathbb{P}^x(E)$ is universally measurable
> 5. For $x\in\mathbb{R}^d$, $\mathbb{P}^x[N_0=x]=1$
> 6. Under $\mathbb{P}^x$, $\tilde{N}=N_t-N_0$ is a Poisson process with intensity $\lambda$.
