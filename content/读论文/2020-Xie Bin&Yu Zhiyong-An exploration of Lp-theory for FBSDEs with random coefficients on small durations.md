...
> [!proposition] 推论3.4（Jacobi矩阵正定时可以得到连续可微的版本）
...
> [!proposition] 推论3.5（无单调性条件的情况下，可以让Lipschitz常数充分小）
...
$$
f(\omega,s,\tilde{\theta},z)=z-\mathbf{G}\sigma(\omega,s,\tilde{\theta},z)-\tilde{z}
$$
我们希望使用全局可测隐函数定理证明$\varphi$的存在性和可测性。由于$\sigma$是$\mathcal{G}\otimes\mathcal{B}(\mathbb{R}^{md})/\mathbb{R}^{md}$可测的，所以$f$是$\mathcal{G}\otimes\mathcal{B}(\mathbb{R}^{n+m+md})/\mathcal{B}(\mathbb{R}^{md})$。我们要验证条件$u$-单调和$u$-Lipschitz连续，取假设(H2)中的满秩矩阵$U$，$z,\bar{z}\in\mathbb{R}^{md}$
$$
\begin{align}
&\left\langle U(f(\omega,s,\tilde{\theta},z)-f(\omega,s,\tilde{\theta},\bar{z})),z-\bar{z}\right\rangle\\
=&\left\langle U((z-\bar{z})-\mathbf{G}(\sigma(\omega,s,\tilde{x},\tilde{y}+G\tilde{x},z)-\sigma(\omega,s,\tilde{x},\tilde{y}+G\tilde{x},\bar{z}))),z-\bar{z}\right\rangle\\
...
\le &|(z-\bar{z})-\mathbf{G}(\sigma(\omega,s,\tilde{\theta},z)-\sigma(\omega,s,\tilde{\theta},\bar{z}))|\\
\le &(1+\|G\|L)|z-\bar{z}|
\end{align}
$$
**QED**
...
> $$
> \left\langle U[I_{md}-\mathbf{G}\frac{\partial\sigma}{\partial z}(\omega,s,\theta)]\hat{z},\hat{z}\right\rangle\ge c|\hat{z}|^2
> $$

> [!proposition] 推论4.3（定理4.2的连续可微版本）
